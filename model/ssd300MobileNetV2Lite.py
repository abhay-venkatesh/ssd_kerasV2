...
def LiteConv(x, i, filter_num):
    ...
    net = Conv2D(
        filter_num, (1, 1), padding="same", use_bias=False, name=str(i) + "_pwconv3"
    )(x)
...
def Conv(x, filter_num):
    net = Conv2D(
        filter_num, kernel_size=1, strides=(2, 2), use_bias=False, name="Conv_1"
    )(x)
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id):
    ...
    x = Conv2D(
        ...
        use_bias=False,
        activation=None,
        name="mobl%d_conv_expand" % block_id,
    )(inputs)
    x = BatchNormalization(
        epsilon=1e-3, momentum=0.999, name="bn%d_conv_bn_expand" % block_id
    )(x)
    ...
    x = DepthwiseConv2D(
        ...
        use_bias=False,
        padding="same",
        name="mobl%d_conv_depthwise" % block_id,
    )(x)
    x = BatchNormalization(
        epsilon=1e-3, momentum=0.999, name="bn%d_conv_depthwise" % block_id
    )(x)
    ...
    x = Conv2D(
        ...
        use_bias=False,
        activation=None,
        name="mobl%d_conv_project" % block_id,
    )(x)
    x = BatchNormalization(
        epsilon=1e-3, momentum=0.999, name="bn%d_conv_bn_project" % block_id
    )(x)
...
def prediction(x, i, num_priors, min_s, max_s, aspect, num_classes, img_size):
    ...
    b = Conv2D(
        num_priors * num_classes, (3, 3), padding="same", name=str(i) + "_mbox_conf"
    )(x)
    mbox_conf_flat = Flatten(name=str(i) + "_mbox_conf_flat")(b)
    mbox_priorbox = PriorBox(
        img_size,
        min_size=min_s,
        max_size=max_s,
        aspect_ratios=aspect,
        variances=[0.1, 0.1, 0.2, 0.2],
        name=str(i) + "_mbox_priorbox",
    )(x)
...
def SSD(input_shape, num_classes):
    ...
    FeatureExtractor = Model(
        inputs=mobilenetv2.input, outputs=mobilenetv2.get_layer("res_connect_12").output
    )
    ...
    predictions = concatenate(
        [mbox_loc, mbox_conf, mbox_priorbox], axis=2, name="predictions"
    )
